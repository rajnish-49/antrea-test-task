...
collect-outputs:
	...
	kubectl exec -n kube-system $$(kubectl get pods -n kube-system -l app=capture-controller -o jsonpath='{.items[0].metadata.name}') -- ls -l /data/capture-* > output/capture-files.txt
	kubectl cp kube-system/$$(kubectl get pods -n kube-system -l app=capture-controller -o jsonpath='{.items[0].metadata.name}'):/data/capture-test-pod.pcap0 output/capture.pcap
	kubectl exec -n kube-system $$(kubectl get pods -n kube-system -l app=capture-controller -o jsonpath='{.items[0].metadata.name}') -- tcpdump -r /data/capture-test-pod.pcap0 > output/capture-output.txt 2>&1
